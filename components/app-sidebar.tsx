...
import { useStageItemCounts, calculateTotalStageCount } from "@/hooks/queries/use-stage-item-counts";
...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  const renderWorkflowStage = useCallback((stage: FetchedWorkflowStage, depth: number = 0) => {
    ...
    // Calculate item count for this stage
    const itemCount = stageCountsData?.stageCountsMap 
      ? calculateTotalStageCount(stage.id, workflowData, stageCountsData.stageCountsMap)
      : 0;
        ...
        {hasSubStages && (
          <CollapsibleContent className="pt-1 space-y-1">
            {stage.sub_stages.map((subStage) => 
              renderWorkflowStage(subStage, depth + 1)
            )}
          </CollapsibleContent>
        )}
      </Collapsible>
    );
  }, [isActive, getIsOpen, toggleCollapsible, getDepthIndicatorColor, stageCountsData, workflowData, isLoadingStageCounts]);
                      ...
                      <>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="font-medium truncate text-sm">
                            {availableSKUs.find(sku => sku.value === selectedSKU)?.label || selectedSKU}
                          </div>
              ...
              <div className="px-3">
                <Link href="/settings" passHref>
                  ...
                  >
                    ...
                    {selectedSKU ? "Configure SKU Workflow" : "Configure Workflow"}
                  </Button>
                </Link>
              </div>
            )}
