-- Fix complete scrap to prevent allocated_quantity from exceeding working_quantity
-- This prevents constraint violation when working_quantity is reduced
...
'Fixed scrap function - handles allocation changes BEFORE updating working_quantity to prevent constraint violations.
- Updates allocations first (triggers reduce allocated_quantity)
- Then updates working_quantity (no constraint violation since allocated_quantity already reduced)
- Complete scrap: Reduces working_quantity after allocated_quantity is adjusted
- Scrap & replace: working_quantity unchanged, allocated_quantity reduced = replacement available';