-- Fix scrap function with simple current stage detection
-- The core issue: we need to get the stage where the item actually IS right now
-- Solution: An item can only be in ONE place at a time, so we get ALL allocations with quantity > 0
-- If there are multiple, we have a data integrity issue, but we'll take the most recent one
...
BEGIN
    ...
    -- Count how many allocations this item has with quantity > 0
    SELECT COUNT(*) INTO v_allocation_count
    FROM item_stage_allocations
    WHERE item_id = p_item_id AND quantity > 0;
    
    IF v_allocation_count = 0 THEN
        RAISE EXCEPTION 'No allocation found with quantity > 0 for item';
    END IF;
    
    IF v_allocation_count > 1 THEN
        RAISE NOTICE 'Item % has % allocations with quantity > 0. Using most recent.', p_item_id, v_allocation_count;
    END IF;
    
    -- Get the current allocation - simply the one with quantity > 0
    -- If multiple exist (data integrity issue), get the most recently created one
    ...
    IF p_quantity > v_current_allocation.quantity THEN
        RAISE EXCEPTION 'Cannot scrap % items. Only % available in current stage.', p_quantity, v_current_allocation.quantity;
    END IF;
