-- Fix the UUID MIN() error in get_sku_workflow_state function
-- PostgreSQL MIN() function doesn't work with UUID types, use ARRAY_AGG instead
...
'Fixed UUID MIN() error: Uses ARRAY_AGG with FILTER to get first stage ID and name for root stages only.
