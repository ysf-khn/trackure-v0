-- Fix the UUID MAX() error in get_sku_workflow_state function
-- PostgreSQL MAX() function doesn't work with UUID types for active_template_id
...
BEGIN
    ...
    templates AS (
        ...
        SELECT 
            ...
            (ARRAY_AGG(t.id) FILTER (WHERE t.is_active = true))[1] as active_template_id,
            (ARRAY_AGG(t.name) FILTER (WHERE t.is_active = true))[1] as active_template_name
        FROM (VALUES (1)) AS dummy(x)
...
'Fixed UUID MAX() and MIN() errors: Uses ARRAY_AGG with FILTER for all UUID aggregations.
