-- Fix scrap_item_with_replacement function to use valid status values
-- The items table status field has a check constraint that only allows specific values
...
BEGIN
    ...
    IF p_quantity = v_current_allocation.quantity THEN
        ...
        SET 
            ...
            -- Keep original status, just mark as scrapped
        WHERE id = p_item_id;
    ...
    ELSE
        ...
        -- Create a new scrapped item entry for tracking (using valid status)
        INSERT INTO items (
            organization_id,
            order_id,
            sku,
        ...
        VALUES (
            ...
            'Completed', -- Use valid status instead of 'Scrapped'  
            true,
            NOW(),
            auth.uid(),
    ...
    IF p_create_replacement THEN
        -- Create new replacement item (with valid status)
        INSERT INTO items (
            organization_id,
            order_id,
            sku,
        ...
        VALUES (
            ...
            'New', -- Valid status for new replacement items
            true,
            scrapped_item_id
        )
