-- Fix allocate_item_to_workflow to allow allocation from items with any status
-- Now that we show unallocated quantities from all statuses, we should be able to allocate them
...
BEGIN
    ...
    -- REMOVED: Status check - we now allow allocation from any status
    -- The important check is whether there's available unallocated quantity
...
-- Update comment
COMMENT ON FUNCTION public.allocate_item_to_workflow(uuid, uuid, integer, uuid) IS 'Allocates unallocated quantity to workflow stage. Works with items of any status as long as they have available unallocated quantity (working_quantity - allocated_quantity > 0). Handles replacement quantities from scrap & replace operations.';