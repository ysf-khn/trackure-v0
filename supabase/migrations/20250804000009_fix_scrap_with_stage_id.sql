-- Fix scrap function to accept stage_id parameter
-- The real issue: Items can have multiple allocations across different stages
-- We need to know WHICH stage the user wants to scrap from!

CREATE OR REPLACE FUNCTION scrap_item_with_replacement(
    p_item_id UUID,
    p_quantity INTEGER,
    p_scrap_reason TEXT,
    p_stage_id UUID,  -- NEW: Which stage to scrap from
...
BEGIN
    ...
    WHERE item_id = p_item_id
      AND stage_id = p_stage_id  -- FIXED: Use the specific stage
      AND quantity > 0;
