-- Create consolidated view for new order items
-- This view aggregates replacement items by order_id + SKU to show single entries
-- instead of multiple replacement items cluttering the new-orders page
...
original_items AS (
    ...
    SELECT 
        ...
        -- Check if any original items still exist (not completely scrapped)
...
scrap_adjustments AS (
    ...
    -- (This reduces the effective original total)
    SELECT 
        i.order_id,
        i.sku,
        i.organization_id,
...
'Consolidated view of new order items that aggregates replacement items by order+SKU. 
Shows single entry per order+SKU combination instead of multiple replacement item rows.
Calculates correct original totals adjusted for complete scraps.';

-- Create index on the underlying tables for better performance
CREATE INDEX IF NOT EXISTS idx_items_replacement_lookup 
ON items(order_id, sku, organization_id, is_replacement, status) 
WHERE status = 'New';

CREATE INDEX IF NOT EXISTS idx_items_scrap_lookup 
ON items(order_id, sku, organization_id, is_scrapped, is_replacement);