-- Fix scrap function to select the correct current allocation
-- The issue was using ORDER BY created_at DESC which selects the most recently created allocation
-- Instead we need to select the allocation where the item currently has quantity > 0
