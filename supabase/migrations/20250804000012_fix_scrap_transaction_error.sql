-- Fix scrap function transaction error
-- Remove COMMIT statement which causes "invalid transaction termination" error
-- PostgreSQL functions cannot use COMMIT/ROLLBACK when called via RPC
...
BEGIN
    -- Ensure transaction consistency without explicit COMMIT
    ...
    -- Transaction will be committed by the calling context
    -- No explicit COMMIT needed (and not allowed in RPC functions)
