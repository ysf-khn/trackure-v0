-- Update allocate_item_to_workflow function to maintain allocated_quantity column
-- This ensures the allocated_quantity column stays in sync with actual allocations
...
-- Update the function comment (specify full signature to avoid ambiguity)
COMMENT ON FUNCTION public.allocate_item_to_workflow(uuid, uuid, integer, uuid) IS 'Allocates a quantity of an item from New status to a workflow stage. Maintains allocated_quantity column for direct unallocated quantity calculation.';